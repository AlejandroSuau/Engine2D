...
// Variables "artísticas" para evitar números mágicos en el códi
...
// Ruido de valor 2D (divide el espacio en celdas, calcula valores en las esquinas e interpola)
float valueNoise(vec2 p) {
    ...
    // Curva de suavizado tipo Hermite
...
// Ruido fractal que suma varias capas con distinta escala / intensidad
...
// Genera ruido animado hacia arriba
...
// Calcula la anchura de la llama según la altura
// (para ser más ancha en base y más estrecha arriba)
float flameWidthAtHeight(float y) {
    return mix(FLAME_BOTTOM_WIDTH, FLAME_TOP_WIDTH, y);
}

// Desplaza ligeramente el centro de la llama con una onda sin()
float flameCenterOffset(float y) {
    return sin(y * 10.0 - uTime * 3.0) * 0.04 * y;
}

// Calcula una coordenada X deformada por el ruido
...
// Crea una máscara de una lengua individual
// Con borde suave y se desvanece progresiva hacia arriba
...
// Crea el cuerpo común para evitar que parezcan 3 columnas separadas.
...
// Máscara final que combina las 3 lenguas, el cuerpo común, los fades y el ruido
...
// Máscara del núcleo brillante interior
...
// Calcula el color de la llama según la altura y el núcleo.
// Predominando abajo amarillo claro y al subir naranja y rojo oscura.
